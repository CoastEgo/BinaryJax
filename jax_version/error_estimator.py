...
def error_critial(i,create,Is_create,parity,deXProde2X,z,de_z):
    pos_idx=jnp.where(((Is_create[i]==1)|(Is_create[i]==10))&(parity[i]==-1*create),size=1)[0]
    neg_idx=jnp.where(((Is_create[i]==1)|(Is_create[i]==10))&(parity[i]==-1*create),size=1)[0]
    z_pos=z[i,pos_idx]
    z_neg=z[i,neg_idx]
...
def no_create_true_fun(carry):
    mag,parab,Is_create,error_hist,parity,deXProde2X,z,de_z=carry
    critial_idx_row=jnp.where((Is_create!=0).any(axis=1),size=error_hist.shape[0],fill_value=-1)[0]
