...
def time_test(t_0,u_0,t_E,rho,q,s,alpha_deg,times,retol,tol):
    ####################编译时间
    uniform_mag,time=timeit(model)(t_0, b, t_E, rho, q, s, alpha_deg, times, tol, retol=retol)
    # with jax.profiler.trace("/tmp/jax-trace", create_perfetto_link=True):
    # jax.profiler.start_trace("/tmp/tensorboard")
        # with jax.disable_jit():
        # model(**parm).block_until_ready()
    # jax.profiler.stop_trace()
    VBBL_mag,_=timeit(VBBL_light_curve)(t_0, b, t_E, rho, q, s, alpha_deg, times, retol=1e-3, tol=1e-3)
...
if __name__=='__main__':
    b = 0.1
    t_0=8280 ;t_E=40 ;alphadeg=270
    q = 0.2; s = 0.9; rho = 10**(-2)
    trajectory_n=1000
    times=jnp.linspace(t_0-1.*t_E,t_0+1.*t_E,trajectory_n)#[idy:idy+1]
