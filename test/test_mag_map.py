...
if __name__=="__main__":
    ...
    tol=1e-3
    t_0=8000;t_E=50
    times=np.linspace(t_0-0.*t_E,t_0+2.0*t_E,trajectory_n);alphadeg=270
    tau=(times-t_0)/t_E
    ...
    mag_vbbl_warp = partial(mag_map_vbbl,all_params=parameter_space,fix_params=[t_0,b_map,t_E,alphadeg,times,tol])
    with Pool(processes=process_number) as pool:
        for VBBL_mag,i in tqdm(pool.imap(mag_vbbl_warp,range(N_Runs)),total=N_Runs,mininterval=1):
    ...
    def mag_jax(i,rho,q,s,parm):
        ...
        uniform_mag,info=model(t_0,b_map[i],t_E,rho,q,s,alphadeg,times_jax,tol=1e-3,retol=1e-3,analytic=False,return_info=True,default_strategy=(30,30,60,120,240,480))
        return uniform_mag,info[-2].sample_num,info[-1].exceed_flag
    for k in range(N_Runs):
        rho,q,s = parameter_space[k]
        jax_map=np.zeros((sample_n,trajectory_n))
        all_nodes=jnp.arange(sample_n);outputs=[];sample_num=[];exceed_flag=[]
        ...
        if k==0:
            ...
            for i in range(sample_n//process_number):
                ...
                mag_i,sample_num_i,exceed_flag_i = jax.pmap(mag_jax,in_axes=(0,None,None,None,None))(slic,rho,q,s,parm)
        ...
        jax_map=jnp.concatenate(outputs,axis=0);sample_num=jnp.concatenate(sample_num,axis=0);exceed_flag=jnp.concatenate(exceed_flag,axis=0)
        ...
        if (k+1)%N_save==0:
            np.savez('test_result/jax_map_test.npz',jax_map_list=jax_map_list,sample_num_list=sample_num_list,exceed_flag_list=exceed_flag_list)
